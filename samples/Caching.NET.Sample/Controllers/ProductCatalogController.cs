...
/// <see cref="IFusionCache"/> — the operation contract Caching.NET exposes — and every
/// configuration concern lives in <c>Program</c> and <c>appsettings.json</c>.
/// </summary>
[ApiController]
...
public sealed class ProductCatalogController : ControllerBase
{
    ...
    private readonly IFusionCache _shortLived;
    ...
    public async Task<ActionResult<Product>> GetAsync(string sku, CancellationToken cancellationToken)
    {
        ...
        var product = await _cache.GetOrSetAsync<Product?>(
            key,
            async token => await _repository.LoadAsync(sku, token),
        ...
        CancellationToken cancellationToken)
    {
        ...
        var products = await _cache.GetOrSetAsync(
            ...
            async token => await _repository.LoadByCategoryAsync(categoryId, token),
            options => options.SetDuration(TimeSpan.FromMinutes(5)).SetEagerRefresh(0.8f),
            tags: tags,
            token: cancellationToken);
    ...
    {
        var remaining = await _shortLived.GetOrSetAsync(
            $"quota:{clientId}",
            async _ => 100,
            token: cancellationToken);
