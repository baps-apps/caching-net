...
internal static class Program
{
    private static async Task<int> Main(string[] args)
    {
        ...
        while ((line = await Console.In.ReadLineAsync()) is not null)
        {
            ...
            try
            {
                var response = await ExecuteAsync(fusion, parts);
                if (response is null)
                {
                    return 0;
                }
    ...
    private static async Task<string?> ExecuteAsync(IFusionCache cache, string[] parts) => parts[0] switch
    {
    ...
    private static async Task<string> SetAsync(IFusionCache cache, string key, string value)
    {
        await cache.SetAsync(key, value);
        return "ok";
    }

    private static async Task<string> SetTaggedAsync(IFusionCache cache, string key, string value, string tag)
    {
        await cache.SetAsync(key, value, tags: [tag]);
        return "ok";
    }

    private static async Task<string> GetAsync(IFusionCache cache, string key)
    {
        var result = await cache.TryGetAsync<string>(key);
        return result.HasValue ? result.Value : "<null>";
    }

    private static async Task<string> PollAsync(IFusionCache cache, string key, string expected, int timeoutMilliseconds)
    {
    ...
    private static async Task<string> PollMissingAsync(IFusionCache cache, string key, int timeoutMilliseconds)
    {
        var deadline = Environment.TickCount64 + timeoutMilliseconds;
        do
        {
    ...
    private static async Task<string> RemoveAsync(IFusionCache cache, string key)
    {
        await cache.RemoveAsync(key);
        return "ok";
    }

    private static async Task<string> RemoveByTagAsync(IFusionCache cache, string tag)
    {
        await cache.RemoveByTagAsync(tag);
        return "ok";
    }

    private static async Task<string> ClearAsync(IFusionCache cache)
    {
