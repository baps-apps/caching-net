...
public class SerializerAndRedactionTests
{
    ...
    {
        // Its own cache name, because the MeterListener is process-wide and "default" is the name
        // almost every other test's cache carries: without this, another test's serializer could
        // satisfy the presence assertions below on this one's behalf.
        const string cacheName = "serializer-metrics";
        using var collector = new Telemetry.MetricCollector();
        var serializer = Build(new FusionCacheSystemTextJsonSerializer(), cacheName: cacheName);

        await serializer.SerializeAsync(new Payload(6, "metric"));

        var own = collector.Own(cacheName);
        Assert.Contains(own, m => m.Instrument == "caching.net.serialization.duration");
        Assert.Contains(own, m => m.Instrument == "caching.net.payload.size");
    }
