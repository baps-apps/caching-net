...
public class CacheExtensionsTests
{
    ...
    /// <summary>
    /// An empty batch must complete without touching the cache — a caller filtering a collection
    /// down to nothing is ordinary, not an error. Asserting on an already-present entry pins the
    /// "no-op" half: an implementation that mistook an empty batch for "write everything" or
    /// "clear" would leave the pre-existing entry gone.
    /// </summary>
    [Fact]
    public async Task SetManyAsync_WithNoItems_IsANoOp()
    {
        using var host = TestHost.BuildInMemory();
        var cache = host.Cache();
        await cache.SetAsync("untouched", 1);

        await cache.SetManyAsync(new Dictionary<string, int>());

        Assert.Equal(1, await cache.GetOrDefaultAsync<int>("untouched"));
    }
