...
public class HybridModeTests
{
    ...
    private CacheHost Host(
        ...
        => CacheHost.Create(cache =>
        {
            ...
            if (distributedExpiration is { } distributed)
            {
                cache.WithDistributedExpiration(distributed);
            }
        });
    ...
    public async Task L1Hit_ServesWithoutTouchingTheFactory()
    {
        ...
        for (var i = 0; i < 5; i++)
        {
            await host.Cache.GetOrSetAsync<int>("Order:1", async (_, _) =>
            {
    ...
    public async Task L1MissWithL2Hit_WarmsL1AndSkipsTheFactory()
    {
        ...
        var first = await reader.Cache.GetOrSetAsync<int>("Order:2", async (_, _) =>
        {
    ...
    public async Task L1AndL2Miss_RunsTheFactoryAndPopulatesBothLayers()
    {
        ...
        var value = await host.Cache.GetOrSetAsync<int>("Order:3", async (_, _) => 7);
    ...
    [Fact]
    public async Task DistributedAndLocalExpirations_CanDiffer_AndTheShorterLocalOneRefetchesFromL2()
    {
        // The two durations are configured explicitly and far apart, so the assertions below can
        // only hold if each one reached its own layer: 150 ms in L1, 30 minutes in L2. That the
        // configuration maps onto the engine at all is asserted directly by CacheEngineMappingTests;
        // what this test adds is the observable consequence against a real Redis.
        using var host = Host(
            "hybrid-l1-expiry",
            backplane: false,
            localExpiration: TimeSpan.FromMilliseconds(150),
            distributedExpiration: TimeSpan.FromMinutes(30));

        var factoryCalls = 0;

        await host.Cache.GetOrSetAsync<int>("Order:4", async (_, _) =>
        {
            Interlocked.Increment(ref factoryCalls);
            return 11;
        });
        ...
        // Past the local lifetime and nowhere near the distributed one: the value comes back, and it
        // cannot have come from L1.
        var value = await host.Cache.GetOrSetAsync<int>("Order:4", async (_, _) =>
        {
            Interlocked.Increment(ref factoryCalls);
            return -1;
        });
        ...
        // A second instance has an empty L1 by construction, so it can only be answered by L2 —
        // which proves the 30-minute distributed lifetime is what is actually stored, rather than
        // the 150 ms local one having been applied to both layers.
        using var fresh = Host(
            "hybrid-l1-expiry",
            backplane: false,
            localExpiration: TimeSpan.FromMilliseconds(150),
            distributedExpiration: TimeSpan.FromMinutes(30));

        Assert.Equal(11, await fresh.Cache.GetOrDefaultAsync<int>("Order:4"));
        Assert.Equal(1, factoryCalls);
    }
    ...
    /// <summary>
    /// The local-expiration bound has to cover tag invalidation, not only plain overwrites.
    /// </summary>
    /// <remarks>
    /// Tag and <c>Clear</c> invalidation are implemented as marker entries that reads compare
    /// themselves against, and a marker is an ordinary cache entry with its own — very long, 10 days
    /// by default — lifetime. Without the local expiration being applied to markers as well, a pod
    /// that had read the key once kept a stale "no marker" copy for those 10 days, so
    /// <c>WithoutABackplane_LocalCopiesAreBoundedByTheLocalExpiration</c> was true for an overwrite
    /// and false for a <c>RemoveByTag</c> — the documented bound covered the cheaper half of the
    /// contract only.
    /// </remarks>
    [Fact]
    public async Task WithoutABackplane_TagInvalidationIsAlsoBoundedByTheLocalExpiration()
    {
        using var podA = Host("hybrid-no-bp-tags", backplane: false, localExpiration: TimeSpan.FromMilliseconds(200));
        using var podB = Host("hybrid-no-bp-tags", backplane: false, localExpiration: TimeSpan.FromMilliseconds(200));

        await podA.Cache.SetAsync("Product:9", 1, tags: ["category:tools"]);

        // Warms pod B, which is what pins the marker copy.
        Assert.True((await podB.Cache.TryGetAsync<int>("Product:9")).HasValue);

        await podA.Cache.RemoveByTagAsync("category:tools");

        var converged = await WaitForAsync(
            async () => !(await podB.Cache.TryGetAsync<int>("Product:9")).HasValue,
            TimeSpan.FromSeconds(10));
    ...
    public async Task PodRestart_RebuildsL1FromL2WithoutRunningTheFactory()
    {
        ...
        {
            await pod.Cache.GetOrSetAsync<int>("Order:8", async (_, _) =>
            {
                Interlocked.Increment(ref factoryCalls);
                return 21;
            });
        ...
        var value = await restarted.Cache.GetOrSetAsync<int>("Order:8", async (_, _) =>
        {
            Interlocked.Increment(ref factoryCalls);
            return -1;
        });
    ...
    // Differing local and distributed lifetimes are covered by
    // DistributedAndLocalExpirations_CanDiffer_AndTheShorterLocalOneRefetchesFromL2, which asserts
    // the observable consequence rather than only that the configuration is accepted.
    ...
    public async Task EagerRefresh_RefreshesInTheBackgroundWithoutBlockingTheCaller()
    {
        ...
        Task<int> Load() => host.Cache.GetOrSetAsync<int>("Order:10", async (_, _) =>
        {
            await Task.Yield();
