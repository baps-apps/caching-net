...
public sealed class RedisSecurityTests : IAsyncLifetime
{
    ...
    {
        // Null-guarded: CreateServerCertificate() runs after _authenticated is assigned and before
        // _tls is, so a failure there used to make disposal throw a NullReferenceException that
        // masked the real setup error — and leak the already-started container it never reached.
        if (_authenticated is not null)
        {
    ...
    /// <summary>
    /// Asserts the write is refused by Redis, and returns the exception so a caller can assert
    /// <i>why</i>.
    /// </summary>
    /// <remarks>
    /// <see cref="RedisException"/> rather than <see cref="Exception"/>: a bare
    /// <c>ThrowsAnyAsync&lt;Exception&gt;</c> passes just as happily on a
    /// <see cref="NullReferenceException"/> from a half-built fixture as on the rejection under
    /// test. The base <c>RedisException</c> rather than <c>RedisConnectionException</c>, because a
    /// loaded machine can surface the same refusal as a command timeout — a different subclass, the
    /// same meaning.
    /// </remarks>
    private static async Task<Exception> AssertRedisRefusesTheWriteAsync(ICacheService cache, string key)
        => await Assert.ThrowsAnyAsync<RedisException>(async () => await cache.SetAsync(key, 1));

    /// <summary>Every message in an exception's <c>InnerException</c> chain, joined.</summary>
    ...
    public async Task WrongPassword_IsNotHiddenFromTheCaller()
    {
        ...
        await AssertRedisRefusesTheWriteAsync(cache, "Order:2");
    }
    ...
    public async Task WrongPassword_DegradesRatherThanFailingTheCallerByDefault()
    {
        ...
        Assert.Equal(5, await cache.GetOrSetAsync<int>("Order:3", async (_, _) => 5));
    ...
    public async Task UntrustedTlsCertificate_IsRejectedUnderStrictValidation()
    {
        ...
        var rejection = await AssertRedisRefusesTheWriteAsync(cache, "Order:4");

        // The connection must fail *because the certificate was rejected*, not merely because the
        // endpoint was unreachable — otherwise a broken container would look like strict validation
        // working. The chain carries an AuthenticationException naming the callback that refused it.
        Assert.Contains("certificate", FlattenMessages(rejection), StringComparison.OrdinalIgnoreCase);
    }
    ...
    public async Task TlsRejection_IsReportedThroughTheTlsValidationMetric()
    {
        ...
        var cache = provider.GetRequiredService<ICacheService>();
        await AssertRedisRefusesTheWriteAsync(cache, "Order:5");
    ...
    public async Task UntrustedTlsCertificate_IsStillRejectedUnderPermissiveValidation()
    {
        ...
        var rejection = await AssertRedisRefusesTheWriteAsync(cache, "Order:5");

        // Same as the strict case: "permissive" must still be a certificate rejection, not merely
    ...
    public async Task TlsRejection_DoesNotTakeTheApplicationDown()
    {
        ...
        Assert.Equal(9, await cache.GetOrSetAsync<int>("Order:6", async (_, _) => 9));
