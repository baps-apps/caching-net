...
- `src/Caching.NET.Analyzers` — Roslyn analyzer (`CardinalityAnalyzer`), shipped as part of the same package
...
- `tests/`:
  ...
  - `Caching.NET.Tests.Analyzers` — analyzer tests
...
- **Resilience / Redis:** `WithResilience(Action<CacheResilienceOptions>)`, `WithRedisOperationTimeout`, `WithStrictCertificateValidation`
