...
internal sealed class RedisConnectionProvider : IDisposable
{
    ...
    public Task<IConnectionMultiplexer> GetConnectionAsync()
    {
        ...
        lock (_gate)
        {
            ...
            // A connect that fails *before* its first await — ConfigurationOptions.Parse rejecting a
            // malformed connection string, or a synchronously-throwing ConnectAsync — returns an
            // already-faulted task, and its catch block ran (and cleared _connectionTask) while this
            // thread was still inside ConnectAsync, before there was anything to clear. Memoizing it
            // here would undo the clear and poison the cache permanently, which is exactly what the
            // catch exists to prevent. Failures after the first await cannot reach this check
            // early: their catch block has to take _gate, which this thread holds until the
    ...
    internal ConfigurationOptions BuildConfiguration()
    {
        ...
        {
            // ConfigurationOptions.CertificateValidation is a multicast RemoteCertificateValidationCallback
            // event handed straight to SslStream as userCertificateValidationCallback. Invoking a
            // multicast delegate with a non-void return type runs every handler but yields only the
            // LAST handler's return value — so attaching Caching.NET's validator and the
            // application's callback as two separate `+=` subscriptions would let an application
            // callback returning true silently override a rejection from
            // _certificateValidator.Validate. Composing them into a single delegate before
            // subscribing keeps our check authoritative: the application callback only runs, and
            // only matters, when Caching.NET's own validation already accepted the certificate, and
            // it can only narrow that result, never widen it.
            var applicationValidation = _options.ValidateServerCertificate;
            ...
            {
                configuration.CertificateValidation += (sender, certificate, chain, errors) =>
                    _certificateValidator.Validate(sender, certificate, chain, errors)
                    && applicationValidation(sender, certificate, chain, errors);
            }
