...
internal sealed class InstrumentedBackplane : IFusionCacheBackplane
{
    ...
    /// <summary>
    /// Wraps <paramref name="backplane"/>, or returns it unchanged when metrics are off so a cache
    /// with telemetry disabled pays nothing at all.
    /// </summary>
    public static IFusionCacheBackplane Wrap(IFusionCacheBackplane backplane, CacheTelemetryContext telemetry)
        => telemetry.MetricsEnabled ? new InstrumentedBackplane(backplane, telemetry) : backplane;
