...
| Backplane spans | ➖ | ➖ | ✅ | `cache.backplane.publish` and `cache.backplane.receive`, both tagged `cache.background_operation=true`. Hybrid only, since it is the only mode that runs a backplane. The receive span also carries `cache.result` and the key the message was for, fingerprinted identically to the publishing instance's span. A message this instance published gets no receive span — Redis delivers it back to the publisher and the engine discards it |
| Health checks | ✅ | ✅ | ✅ | |
