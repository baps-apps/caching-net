...
> **HISTORICAL — partially superseded.** Task 10 (`Caching.NET.Analyzers` / `CN0001` cardinality Roslyn analyzer) and the `Caching.NET.Tests.Analyzers` test project were **removed in v2.1.0**. The forbidden tag/log-template name set (`key`, `tenant`, `user_id`, `cache.*` variants) is now a runtime convention, not compile-time enforced. Ignore the analyzer build / pack / test instructions below.
>
