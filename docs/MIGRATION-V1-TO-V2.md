...
| v1 | v2 | Notes |
|----|----|-------|
| `"Caching": { ... }` in `appsettings.json` (incorrect snippet) | `"CacheOptions": { ... }` | The library has always bound **`CacheOptions`** (`CacheConfigurationKeys.CacheOptions`); fix mis-copied JSON and env vars (`CacheOptions__` not `Caching__`) |
