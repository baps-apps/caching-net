...
On **`Counter` / `Histogram` / `UpDownCounter`** `.Add` / `.Record` calls that resolve to `System.Diagnostics.Metrics` types, and on **`ILogger`** calls where the message template / `BeginScope` format is a **compile-time string literal** in that invocation:
